...
import static info.jerrinot.loomexperiment.mess.ThreadSchedulerStrategy.VIRTUAL_DEFAULT_SCHED;
 ...
 */
public final class DataFlowExperiments {
    ...
    private static final ThreadSchedulerStrategy THREAD_SCHEDULER = VIRTUAL_DEFAULT_SCHED;
    ...
    public void dataFlow_spinning() throws InterruptedException {
        ...
        for (int i = 0; i < PIPELINE_LENGTH - 1; i++) {
            ...
            for (int w = 0; w < WORKERS_PER_STAGE; w++) {
                taskExecutor.startTask(new SpinningQueuePump(src, dst));
            }
        }
